...
async function sendMint(plan, signer, contract, gasConfig, dryRun) {
  const { fnName, args, value, calldata, gasEstimate, candidate } = plan;
  const isRaw = candidate.type === 'rawSelector';
  ...
  if (isRaw) {
    logger.info(`  selector:   ${candidate.selector} (raw)`);
    logger.info(`  argTypes:   [${(candidate.argTypes || []).join(', ')}]`);
    logger.info(`  argMode:    [${candidate.argKeys.join(', ')}]`);
    logger.info(`  args:       [${args.map(a => Array.isArray(a) ? `[${a.length} proofs]` : String(a)).join(', ')}]`);
  } else {
  ...
  try {
    if (isRaw) {
      // Raw selector: send transaction with raw calldata
