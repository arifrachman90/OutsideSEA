...
async function simulatePlan(plan, contract, from, provider) {
  ...
  const label = isRaw
    ? `[Raw Selector] ${candidate.selector} [${(candidate.argTypes || []).join(', ')}] | mode: [${candidate.argKeys.join(', ')}] | args: [${args.map(a => Array.isArray(a) ? `[${a.length} proofs]` : String(a)).join(', ')}]`
  ...
  try {
    if (isRaw) {
      // Raw selector: use provider.call with raw calldata
