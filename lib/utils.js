...
function printSimulationReport(results) {
  ...
  for (const r of results) {
    ...
    const isRaw  = r.candidate.type === 'rawSelector';
    const label  = isRaw
      ? `${r.candidate.selector} [${(r.candidate.argTypes || []).join(', ')}] (raw)`
      : `${r.fnName}(${r.candidate.argKeys.join(', ')})`;
    console.log(
      `  [${String(r.candidate.id).padStart(2)}] ${status} | ${gas.padEnd(18)} | ${label}${reason}`
    );
  }
  console.log('─'.repeat(80));
}
...
module.exports = { truncateHex, fmtBigInt, resolveMintTo, selectBestCandidate, printSimulationReport, sleep };
